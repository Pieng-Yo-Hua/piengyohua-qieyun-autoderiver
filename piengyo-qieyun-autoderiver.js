...
function 聲母規則(文讀) {
    ...
    if (is('孃母 三等')) return 'nz'; //孃母待攷，你字當特別処理

    if (is('精母')) return 'z';
    if (is('清母')) return 'c';
    if (is('從母')) return is('平聲') && 文讀 ? 'c' : 'z';
    if (is('心母')) return 's';
    if (is('邪母')) return is('平聲') && 文讀 ? 'c' : 's';
    ...
    if (is('崇母') && !文讀) return is('臻梗攝') ? 'zh' : 'z';
    ...
    if (is('曉匣母')) return 'h';
    if (is('影母')) return is('一二等') ? 'ng' : ''; //只見于開口呼
    if (is('云以母')) return '';
...
function 韻母規則(文讀) {
    ...
    if (is('曾攝 舒聲') && !文讀) {
        if (is('蒸韻')) return is('知莊章組 或 日母') ? 'r' : 'i';
        if (is('登韻')) return is('合口') ? 'ueng' : 'eng';
    }
    if (is('曾攝 入聲')) {
        if (is('蒸韻 合口')) return 'yuaq';
        if (is('蒸韻')) return is('知莊章組 或 日母') ? 'aq' : 'iaq'; //影母字待考
